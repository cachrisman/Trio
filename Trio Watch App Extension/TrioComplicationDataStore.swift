...
struct TrioComplicationSnapshot: Equatable {
    ...
    init?(dictionary: [String: Any]) {
        ...
        if let date = dictionary["date"] as? Date {
            dateValue = date
        } else if let seconds = dictionary["date"] as? TimeInterval {
            dateValue = Date(timeIntervalSince1970: seconds)
        } else {
            return nil
        }

        let readingDateValue: Date
        if let readingDate = dictionary["readingDate"] as? Date {
            readingDateValue = readingDate
        } else if let seconds = dictionary["readingDate"] as? TimeInterval {
            readingDateValue = Date(timeIntervalSince1970: seconds)
        } else {
            return nil
        }
        ...
        let glucoseColorValue = dictionary["glucoseColor"] as? String
    ...
    func toDictionary() -> [String: Any] {
        var dict: [String: Any] = [
            ...
            "trend": trend,
            "delta": delta,
            "date": date, // Use Date directly instead of TimeInterval
            "readingDate": readingDate // Use Date directly instead of TimeInterval
        ]
        if let state = state {
            dict["state"] = state
        }
        if let glucoseColor = glucoseColor {
            dict["glucoseColor"] = glucoseColor
        }
        return dict
    }
...
final class TrioComplicationDataStore {
    ...
    ///   - readingDate: The timestamp of the reading.
    ///   - date: The timestamp when the snapshot was created.
    ///   - glucoseColor: The hex color string for the glucose value (optional).
    func save(glucose: String, trend: String?, delta: String?, readingDate: Date, date: Date, glucoseColor: String? = nil) {
    ...
    func save(_ snapshot: TrioComplicationSnapshot) {
        ...
        do {
            ...
            let bakURL = fileURL.deletingLastPathComponent().appendingPathComponent("snapshot.bak")
            if fileManager.fileExists(atPath: fileURL.path) {
                _ = try? fileManager.removeItem(at: bakURL)
                _ = try? fileManager.copyItem(at: fileURL, to: bakURL)
            }
            do {
                try data.write(to: fileURL, options: [.atomic])
                Self.lastValidTimestamp = snapshot.date
                #if os(watchOS)
                    Task { await WatchLogger.shared.log("✅ Saved complication snapshot to \(fileURL.lastPathComponent)") }
                #endif
                #if os(iOS)
                ...
                // Immediately reload the timeline to update the complication
                coalescedReload()

                // Also schedule a delayed reload as backup to ensure the complication updates
                ...
                if fileManager.fileExists(atPath: bakURL.path) {
                    ...
                    _ = try? fileManager.copyItem(at: bakURL, to: fileURL)
                }
            }
        } catch {
    ...
    func coalescedReload(minInterval: TimeInterval = 30) {
        let now = Date()
        guard now.timeIntervalSince(lastReload) >= minInterval else { return }
        lastReload = now
        reloadTimeline()
    }
    ...
    func latestSnapshot() -> TrioComplicationSnapshot? {
        ...
        func decode(from url: URL) throws -> TrioComplicationSnapshot {
            ...
            let wrapped = try decoder.decode(SnapshotCodable.self, from: data)
            Self.lastValidTimestamp = wrapped.snapshot.readingDate
            return TrioComplicationSnapshot(
                glucose: wrapped.snapshot.glucose,
                trend: wrapped.snapshot.trend,
                delta: wrapped.snapshot.delta,
                readingDate: wrapped.snapshot.readingDate,
                date: wrapped.snapshot.date,
                state: nil
            )
        }
        ...
        do {
            ...
            let bakURL = fileURL.deletingLastPathComponent().appendingPathComponent("snapshot.bak")
            if fileManager.fileExists(atPath: bakURL.path), let snap = try? decode(from: bakURL) {
                #if os(watchOS)
                    Task { await WatchLogger.shared.log("⚠️ Using backup snapshot") }
                #else
                    NSLog("%@", "⚠️ Using backup snapshot")
                #endif
                return snap
            }
    ...
    /// Last timeline reload timestamp for throttling
    private static var lastTimelineReload: Date?

    /// Minimum interval between timeline reloads (optimized for 5-minute CGM cycle)
    private static let timelineReloadThrottleInterval: TimeInterval = 60.0

    #if canImport(WidgetKit)
        /// Reloads the WidgetKit complication timeline for the Trio complication.
        /// Includes throttling to prevent excessive reloads.
        func reloadTimeline() {
            let now = Date()

            // Check if we should throttle this reload
            if let lastReload = Self.lastTimelineReload,
               now.timeIntervalSince(lastReload) < Self.timelineReloadThrottleInterval
            {
                #if os(watchOS)
                    Task {
                        await WatchLogger.shared
                            .log("📅 Timeline reload throttled (last reload: \(now.timeIntervalSince(lastReload))s ago)")
                    }
                #endif
                return
            }
            ...
            Self.lastTimelineReload = now

            #if os(watchOS)
                Task { await WatchLogger.shared.log("📅 Reload requested at \(now)") }
            #endif
            let reloadBlock = {
                if #available(watchOS 10.0, *) {
                        ...
                        Task {
                            await WatchLogger.shared
                                .log("📅 Triggered WidgetCenter.shared.reloadTimelines for \(Self.complicationKind) at \(Date())")
                        }
                    #endif
