...
@Observable final class WatchState: NSObject, WCSessionDelegate {
    ...
    private func saveComplicationSnapshot(from message: [String: Any], fromUserInfo: Bool = false, userInfoReceiveTimestamp: Date? = nil) {
        ...
        TrioComplicationDataStore.shared.save(snapshot, minInterval: 5)
        // Beacon hook fires here, which is *before* the data store's `minInterval=5` debounce. The
        // beacon's own `readingDate`-based dedup (`HapticBeacon.lastAcceptedReadingDate`) is what
        // protects against batched / replayed WC deliveries — the data store's debounce does not.
        let wcReadingDate = readingDate
        Task { @MainActor in
            HapticBeacon.shared.noteEGVReceived(at: Date(), readingDate: wcReadingDate, source: .watchConnectivity)
        }
