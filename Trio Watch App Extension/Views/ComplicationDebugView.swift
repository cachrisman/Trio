...
struct ComplicationDebugView: View {
    ...
    /// Mirror of `HapticBeacon.shared.sourceFilter` so the source-filter button label re-renders
    /// after a tap. Same `.onAppear` initialization pattern as `hapticBeaconEnabled`.
    @State private var hapticBeaconSourceFilter: HapticBeacon.SourceFilter = .ble
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            hapticBeaconSourceFilter = HapticBeacon.shared.sourceFilter
        }
    ...
    private var actionsView: some View {
        VStack(spacing: 8) {
            ...
            Button {
                let newValue: HapticBeacon.SourceFilter = hapticBeaconSourceFilter == .ble ? .bleAndRelayedFallback : .ble
                HapticBeacon.shared.setSourceFilter(newValue)
                hapticBeaconSourceFilter = newValue
                triggerConfirmation(
                    message: newValue == .bleAndRelayedFallback ? "📡 Haptic Source: BLE + relayed fallback" : "📡 Haptic Source: BLE only"
                )
            } label: {
                HStack {
                    Image(systemName: hapticBeaconSourceFilter == .bleAndRelayedFallback ? "antenna.radiowaves.left.and.right" : "dot.radiowaves.left.and.right")
                    Text("Source: \(hapticBeaconSourceFilter == .bleAndRelayedFallback ? "BLE + relayed" : "BLE only")")
                }
                .frame(maxWidth: .infinity)
            }
            .buttonStyle(.bordered)
