...
final class WatchGlucoseHistoryStore {
    ...
    /// C-209-9 (review 6.8): `queue.async` — callers (often main / the BLE path) no longer
    /// block on the JSON encode + atomic write. Reads stay `queue.sync`, so a read issued
    /// after an insert still observes it (serial queue ordering).
    ...
    /// C-209-9 (review 6.8): async, same rationale as the single-reading insert.
