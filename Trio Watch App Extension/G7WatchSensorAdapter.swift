...
final class G7WatchSensorAdapter: NSObject {
    ...
    ///   1. `stop()` — when tearing down `extendedSession` / `sessionPendingDidStart`.
    ///
    ...
    // W7 slot accounting: `expectedSlotsToday` = eligible 5-min slots since midnight (monotonic
    // high-water via `lastExpectedSlotEpoch`); `gatedSlotEpochsToday` = the subset C1 deferred and
    // hasn't yet captured (a SET so a slot can be un-gated by a later capture). Debug shows
    // N / (expected − gated). All accumulate across a sensor swap; reset only on day rollover.
    private var expectedSlotsToday: Int = 0
    private var gatedSlotsToday: Int = 0 // mirror of gatedSlotEpochsToday.count for display
    private var lastExpectedSlotEpoch: Int = -1
    ...
    private enum Keys {
        ...
        static let lastExpectedSlotEpoch = "G7WatchAdapter.lastExpectedSlotEpoch"
    ...
    private func emitExpectedWindowTick(epoch: Int, retroactive: Bool) {
        ...
        if eligible {
            ...
            // Monotonic high-water (`>`, not `!=`): cold-start replays re-emit already-counted slots;
            // `>` skips them so they aren't double-counted. Today-only so a cross-midnight replay
            // counts only post-midnight slots.
            if slotEpoch >= startOfToday, slotEpoch > lastExpectedSlotEpoch {
    ...
    private func loadDailyCounters() {
        ...
        if storedDay != dayStart {
            ...
            expectedSlotsToday = UserDefaults.standard.integer(forKey: Keys.expectedSlots)
            lastExpectedSlotEpoch = (UserDefaults.standard.object(forKey: Keys.lastExpectedSlotEpoch) as? Int) ?? -1
    ...
    private func resetDailyCountersInMemory() {
        bleConnectsToday = 0
        bleEGVsToday = 0
        expectedSlotsToday = 0
        lastExpectedSlotEpoch = -1
    ...
    private func persistDailyCounters() {
        ...
        UserDefaults.standard.set(expectedSlotsToday, forKey: Keys.expectedSlots)
        UserDefaults.standard.set(lastExpectedSlotEpoch, forKey: Keys.lastExpectedSlotEpoch)
    ...
    private func mirrorDailyCountersToWatchState() {
        ...
        WatchState.shared.expectedSlotsToday = expectedSlotsToday
    ...
    /// C1 — the ONLY way the adapter starts scanning/connecting. Every scan entry routes here.
    ...
    private func publishConnectionStatus() {
        ...
        if s.isConnected {
            ...
            // UI-207-2: armed but not connected/scanning/stopped — the idle wait between G7
            // advertisements (incl. the fork's 2s post-disconnect settle and a C1-deferred scan),
            // not an OS-cache "retrieving" operation.
            status = .waiting
        }
...
extension G7WatchSensorAdapter: G7SensorDelegate {
    ...
    private func handleSensorDidRead(glucose: G7GlucoseMessage) {
        ...
        // Persist to the rolling history store (W1) BEFORE the snapshot apply, so the chart refresh
        // inside applyG7DirectBleSnapshot includes this reading. BLE readings are always valid mg/dL.
        WatchGlucoseHistoryStore.shared.insert(
            StoredGlucoseReading(epochSeconds: readingEpoch, glucoseMgDl: glucoseValue, sequence: currentSequence, source: "ble")
        )
        ...
        let snapshot = TrioComplicationSnapshot(
            ...
            date: Date(),
            state: nil,
            glucoseColor: WatchGlucoseColorComputer.shared.bubbleColorHex(for: glucoseValue), // W5: bake bubble hex
            source: .g7DirectBLE,
...
extension G7WatchSensorAdapter: WKExtendedRuntimeSessionDelegate {
    ...
    func extendedRuntimeSession(
        ...
        if session === extendedSession { extendedSession = nil }
        ...
        if hasError {
            // C-207-1: the system kills the foreground-only WKExtendedRuntimeSession with a
            // RunningBoard assertion (NSError domain == "RBSAssertionErrorDomain") every time the app
            // leaves the active scene. Build-206 telemetry: ~45% of EGVs arrive while backgrounded via
            // the BLE central, so tearing the pipeline down here (stop() -> sensor.stopScanning(), with
            // the recovery gated on scene == active and therefore skipped in the background) blanks BLE
            // exactly when the next reading is due. Treat that specific background invalidation as
            // benign: the live session reference is already cleared above, so just leave scanning
            // running. Only a genuine (non-RBS) error, or an invalidation while foreground-active, tears
            // down. Match on the NSError domain, not reason.rawValue (lastKnownScenePhase is a String).
            ...
            log("ext_session_unexpected_invalidation", "triggering_teardown=true \(detail)")
            stop()
