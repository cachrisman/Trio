...
final class G7WatchSensorAdapter: NSObject {
    ...
    /// not on every foreground active entry.
    ...
    /// setup pass per process. The retroactive `expected_window` replay is further
    /// gated by `hasAnchored` so it runs once per process.
    func start() {
