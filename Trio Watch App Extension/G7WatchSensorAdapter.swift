...
final class G7WatchSensorAdapter: NSObject {
    ...
    private override init() {
        // Always construct in scan mode; identity is established via `didDiscoverNewSensor` once
        // the phone has pushed an `expectedSensorName` (or, on cold start, from UserDefaults).
        sensor = G7Sensor(sensorID: nil)
        super.init()
        sensor.delegate = self
        loadDailyCounters()
        if expectedSensorName != nil {
...
extension G7WatchSensorAdapter: WKExtendedRuntimeSessionDelegate {
    func extendedRuntimeSessionWillExpire(_ session: WKExtendedRuntimeSession) {
        ...
        // chain attempt is still in flight.
