...
final class BaseWatchManager: NSObject, WCSessionDelegate, Injectable, WatchManager {
    ...
    private func fetchGlucose() async throws -> [NSManagedObjectID] {
        let results = try await CoreDataStack.shared.fetchEntitiesAsync(
            ...
            predicate: NSPredicate.glucoseForTwoHoursAgo, // build 205 / P1: ~2h window (watch keeps full 24h locally)
            key: "date",
            ascending: false,
            fetchLimit: 24
        )
    ...
    func watchStateToDictionary(from state: WatchState) -> [String: Any] {
        ...
        dict[WatchMessageKeys.lowGlucoseThreshold] = NSDecimalNumber(decimal: lowGlucose).intValue
        dict[WatchMessageKeys.highGlucoseThreshold] = NSDecimalNumber(decimal: highGlucose).intValue
        dict[WatchMessageKeys.glucoseTarget] = NSDecimalNumber(decimal: currentGlucoseTarget).intValue
        ...
        if let currentMgDl = state.currentGlucoseMgDl {
            dict[WatchMessageKeys.currentGlucoseMgDl] = currentMgDl
        }
        ...
        if state.g7ActivationEpoch > 0 {
            dict[WatchMessageKeys.g7ActivationEpoch] = state.g7ActivationEpoch // C2: Int64 seconds; omitted when none (watch treats absence as legacy name-only)
        }
