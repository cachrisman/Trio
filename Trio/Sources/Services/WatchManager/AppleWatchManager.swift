...
final class BaseWatchManager: NSObject, WCSessionDelegate, Injectable, WatchManager {
    ...
    @MainActor func sendDataToWatch(
        ...
        let complicationAllowlist: [(String, String)] = [
            ...
            // Glucose Bobble complication (TRIO-063): the loop-status ring ages from lastLoopDate, so it
            // must ride the background channels too or the ring turns red whenever the watch app is
            // not in the foreground; the settings are small and change rarely.
            (WatchMessageKeys.lastLoopDate, "lastLoopDate"),
            (WatchMessageKeys.glucoseBobbleComplicationSettings, "glucoseBobbleComplicationSettings")
        ]
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let type = message["type"] as? String,
               let payloadId = message["payloadId"] as? String {
                ...
                // TRIO-047: skip payloads already handled (the watch resends what it believes
                // unacknowledged), then handle, then record — the same handle-then-record order as
                // the replyHandler variant (TRIO-043), so termination mid-handling yields a duplicate
                // on resend rather than a loss. There is no ACK on this path. Check and record both
                // run here on the main queue. A torn-down manager handles and records nothing, so a
                // resend is handled later. Only a payload actually handled is recorded: an unknown
                // type or missing data must not block a later valid delivery of the same payloadId.
                guard let self else { return }
                if self.isProcessed(payloadId) { return }
                var handled = false
                if type == "watchLogs" {
                    if let logData = message["data"] as? String {
                        SimpleLogReporter.appendToWatchLog(logData)
