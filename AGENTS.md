...
```bash
./generate-patch.sh -n -d "short-description"
```
...
# During cutover only (temporary): use .am.patch if the repo is still in the transition window
# git format-patch -1 --stdout HEAD > patches/NN-short-description.am.patch
# git am --check patches/NN-short-description.am.patch
```
...
Note: this is only relevant during migration. Post-cutover, patches should already be mailbox patches generated by `generate-patch.sh`.

```bash
git checkout dev
git checkout -b tmp/convert
git apply --whitespace=fix patches/NN-something.patch
git commit -am "feat(something): short summary" || git commit -m "feat(something): short summary"
git format-patch -1 --stdout HEAD > patches/NN-something.am.patch
git checkout dev
git branch -D tmp/convert
```
