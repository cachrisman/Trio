...
final class TrioComplicationDataStore {
    ...
    private var appGroupDefaults: UserDefaults? {
        ...
        if let classBundle = Bundle(for: type(of: self)) as Bundle? {
        ...
        guard let suiteName = bundle.object(forInfoDictionaryKey: "AppGroupID") as? String else {
            // Log once when App Group is unavailable
            if !Self.hasLoggedAppGroupUnavailable {
                Self.hasLoggedAppGroupUnavailable = true
                log("⚠️ AppGroupID not found in Info.plist (bundle: \(bundleSource), ID: \(bundleID)); using in-memory fallbacks for debounce/token")
            }
            return nil
        }

        log("✓ appGroupDefaults: Found AppGroupID: \(suiteName)")
    ...
    var appGroupID: String? {
        ...
        return bundle.object(forInfoDictionaryKey: "AppGroupID") as? String
    }
    ...
    private static func defaultSharedContainerURL() -> URL? {
        ...
        // Try to find the extension bundle by looking for the class's bundle
        if let classBundle = Bundle(for: TrioComplicationDataStore.self) as Bundle? {
        ...
        guard let suiteName = bundle.object(forInfoDictionaryKey: "AppGroupID") as? String else {
            NSLog("[ComplicationDataStore] ❌ AppGroupID not found in bundle %@ (ID: %@)", bundleSource, bundleID)
            return nil
        }

        NSLog("[ComplicationDataStore] ✓ Found AppGroupID: %@", suiteName)
