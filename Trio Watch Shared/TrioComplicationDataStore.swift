    ...
    private var appGroupDefaults: UserDefaults? {
        guard let suiteName = Bundle.main.object(forInfoDictionaryKey: "AppGroupID") as? String else {
            ...
            if !Self.hasLoggedAppGroupUnavailable {
                ...
                log("⚠️ AppGroupID not found in Info.plist; using in-memory fallbacks for debounce/token")
            }
            return nil
        }
        let defaults = UserDefaults(suiteName: suiteName)
        if defaults == nil && !Self.hasLoggedAppGroupUnavailable {
            Self.hasLoggedAppGroupUnavailable = true
            log("⚠️ App Group UserDefaults unavailable; using in-memory fallbacks for debounce/token")
        }
        return defaults
    }
